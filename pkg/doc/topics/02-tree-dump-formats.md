---
Title: Tree Dump Formats in Oak
Slug: tree-dump-formats
Short: Compare and use Oak's text, XML, JSON, and YAML tree dump outputs.
Topics:
- oak
- tree-sitter
- ast
...
Oak supports human-friendly and machine-friendly formats so you can pick the right tool for debugging, visualization, or downstream processing.
...
Use the `parse` command with `--dump-format` to select an output format and refine it with additional flags.
...
Control verbosity and attributes to tailor output for your use case.
...
Integrate dumpers into Go programs via Oak command helpers or the low-level API.
...
All formats share consistent position semantics; this section highlights per-format structure and examples.
...
Each dumper implements a common interface; higher-level helpers wire them into Oak commands.
...
Choose formats based on readability requirements, integration targets, and size constraints.
...
Multiple output formats give you flexibility from quick inspection to robust automation.
